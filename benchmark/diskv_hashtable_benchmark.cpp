...
int main(int argc, char* argv[]) {
    ...
    diskv::HashTable *index = new diskv::HashTable(disk, 15);
...

    {
        ...
        for (int i = 0; i < data_num; i++) {
            ...
            if (memcmp(value.data_, res_value.data_, VALUE_SIZE) != 0) {
                std::cout << "HashTable execute " << i << "th queury error: correct value - " << std::string(res_value.data_, VALUE_SIZE) << " get value - " << std::string(value.data_, VALUE_SIZE) << std::endl;
            }
        }
